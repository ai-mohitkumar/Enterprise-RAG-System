        ...
        for i, (doc_id, doc) in enumerate(list(self.documents.items())[:top_k]):
            results.append(
                RetrievedChunk(
                    ...
                    similarity_score=0.95 - (i * 0.05),  # Mock decreasing scores
...
class ProductionRAGSystem:
    """
    Production-grade RAG system with hallucination prevention,
    security validation, and comprehensive audit logging.
    """
    ...
    def __init__(
        ...
        self.model = model
        self.temperature = temperature  # Low temperature to reduce hallucinations
    ...
    def process_query(
        ...
        if not retrieved_chunks:
            ...
            return self._create_response(
                query, response, ResponseType.REFUSED, retrieved_chunks,
                0.0, [], [], start_time, user_id
            )

        # Step 4: Validate grounding potential
        confidence, is_grounded, risk_assessment = \
            self.hallucination_detector.validate_response_grounding(
                context, context, query
            )

        # Step 5: Generate LLM response
        llm_response, tokens_used = self._generate_response(
            query, context, retrieved_chunks
        )

        # Step 6: Validate response
        response_confidence, is_valid_response, validation_msg = \
            self.hallucination_detector.validate_response_grounding(
        ...
        if response_confidence >= 0.8:
            ...
            llm_response += "\n\n⚠️ **Note**: This answer has low confidence. " \
        ...
        response = self._create_response(
            query, llm_response, response_type, retrieved_chunks,
            response_confidence, citations, [c["source"] for c in citations],
            start_time, user_id, response_confidence
        )

        # Log audit trail
        self._audit_log(response, tokens_used, validation_msg)

        return response

    def _generate_embedding(self, text: str) -> List[float]:
        """Generate embedding for text (mock implementation)."""
        # In production, use: response = openai.Embedding.create(...)
        # For now, return mock embedding
        import hashlib
        hash_obj = hashlib.md5(text.encode())
        hash_hex = hash_obj.hexdigest()
        # Convert hash to float embedding (mock)
...
2. If information is not in the documents, say "I don't have information about that in the provided documents"
